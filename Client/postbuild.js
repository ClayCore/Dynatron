        ...
        if (files) {
            ...
            html.forEach((file) => {
                staticAssets.forEach((name) => {
                    let options = {
                        files: path.join('build/release', file),
                        from: new RegExp(escapeRegExp(name), 'g'),
                        to: staticDir + '/' + name,
                    };
                    try {
                        let changedFiles = replace.sync(options);
                        console.log('Modified files:', changedFiles.ToString().join(', '));
                    } catch (error) {
            ...
            assets.forEach((name) => {
                fs.rename(
                    ...
                    path.join(__dirname, 'build/release', assetsDir, name),
                    function (err) {
                        if (err) throw err;
            ...
            staticAssets.forEach((name) => {
                fs.rename(
                    path.join(__dirname, 'build/release', name),
                    path.join(__dirname, 'build/release', staticDir, name),
                    function (err) {
                        if (err) throw err;
                        console.log(`Successfully moved ${name}`);
                    }
